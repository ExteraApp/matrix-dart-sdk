...
class MatrixSdkDatabase extends DatabaseApi with DatabaseFileStorage {
  ...
  Future<void> open() async {
    ...
    _securityIncidentsBox = _collection.openBox(_securityIncidentsBoxName);
  ...
  Future<List<Event>> getThreadEventList(
    ...
    if (start != 0) {
      ...
      final sendingTimelineKey = TupleKey(
        thread.room.id,
        'SENDING',
        thread.rootEvent.eventId,
      ).toString();
  ...
  Future<void> setThreadPrevBatch(
    ...
    final raw = await _threadsBox.get(
      TupleKey(roomId, threadRootEventId).toString(),
    );
    if (raw == null) return;
